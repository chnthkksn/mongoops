...
- **Database Access (`src/database-users/`)**: manages real MongoDB user accounts on a target cluster (`createUser`/`updateUser`/`dropUser`/`usersInfo` run against that cluster's `admin` db via the driver, `forAllDBs: true` for cross-db listing) — distinct from better-auth's platform users. These are live cluster credentials, so the whole module is owner-only, one tier stricter than the usual owner/admin mutation split. Password reset accepts an optional caller-supplied password (`ResetDatabaseUserPasswordDto.password`), falling back to a generated one when omitted.
...
- Standard component conventions already established: shadcn `Dialog`/`Table`/`Button` from `src/components/ui/`, `cn()` from `src/lib/utils.ts` for conditional classes.
- **Destructive-action confirmations**: use `useConfirm()` from `src/components/ui/confirm-dialog.tsx` (`const confirm = useConfirm(); const ok = await confirm({ title, description, confirmLabel? }); if (!ok) return;`) — a shared `ConfirmProvider` (mounted in `layout.tsx`) renders one dialog for the whole app. Never use the native `confirm()`; every destructive action in the app was migrated off it. Defaults to a destructive-red confirm button; pass `variant: "default"` to opt out.
- **Dark mode**: `next-themes` (`ThemeProvider` in `src/components/theme-provider.tsx`, `attribute="class"`, `defaultTheme="system"`) wraps the app in `layout.tsx`; toggle lives in `src/components/theme-toggle.tsx` in the app shell header. Colors are CSS custom properties (oklch) in `src/app/globals.css` — `.dark` only overrides variables it explicitly redefines, so a new color token must be added to **both** `:root` and `.dark` or it silently keeps its light value when the class flips.
- **Cluster color coding**: each cluster has a user-picked `color`; active-cluster pill chips reuse it directly via inline `style={{ backgroundColor: cluster.color }}` (see `src/lib/cluster-colors.ts` for the picker palette) rather than a fixed `bg-primary` — applied consistently across Monitoring, Database Access, Backup, and Database Explorer's cluster selectors.
